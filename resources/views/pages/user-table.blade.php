                            ...
                            <tbody>
                                @foreach ($users as $index => $user)
                                    <tr>
                                        ...
                                        <td>{{ $user->role ?? '-' }}</td>
                                        <td>
                                            <button class="btn btn-sm btn-warning editBtn" eid="{{ $user->id }}" erole="{{ $user->role }}" editName="{{ $user->name }}" editUname="{{ $user->email }}" value="{{ $user->dpm }}"><i class="bi bi-gear"></i></button>
            ...
            $(".addBtn").click(function() {
                Swal.fire({
                    ...
                    html: `
                        ...
                        <select class="form-select mb-3" aria-label="Default select example" id="dpm">
                    ...
                    preConfirm: () => {
                        ...
                        if (name && uname && pass && dpm && role) {
                            $.ajax({
                                ...
                                data: { name:name, uname:uname, pass:pass, dpm:dpm, role:role},
                ...
                Swal.fire({
                    ...
                    html: `
                        ...
                        <select class="form-select mb-3" aria-label="Default select example" id="dpm">
                            <option value="" selected disabled>Select department</option>
                            @foreach ($dpms as $dpm)
                                <option value="{{ $dpm->id }}" ${dpm == "{{ $dpm->id }}" ? 'selected' : ''}>{{ $dpm->name }}</option>
                            @endforeach
                        </select>
                        <select class="form-select" aria-label="selectRole" id="role">
                            <option value="" selected disabled>Select role</option>
                    ...
                    preConfirm: (agnName) => {
                        ...
                        if (name && uname && dpm) {
                            $.ajax({
                                ...
                                data: { name:name, uname:uname, pass:pass, dpm:dpm, eid:eid, role:role},
                                success: function (response) {
                                    // console.log(response);
                                    Swal.fire({
                                        title: "Success",
